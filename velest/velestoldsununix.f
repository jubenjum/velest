      program velest
c
c     version 3.2  E.Kissling, Institute of Geophysics, ETH Zurich
c
c                  21. April 1995
c
...
c                              write on file 12
     ...
     &                   ptcor(m),stcor(m)
            ...
            if(m.eq.1)then
               cline(47:)='st,lat,lon,z,model,icc,ptcor,stcor'
            endif
            write(12,'(a)') cline
         endif
c                            end writing on file 12
         if(dd(m).eq.0.or.icc(m).lt.2) goto 13
         bb(m)=(bb(m)-aa(m)**2/dd(m))*icc(m)/(dd(m)*(icc(m)-1))
         iwarn=0
         if(bb(m).lt.0)then
           iwarn=1
           write(16,'(1x,''WARNING:   Station = '',a4,
     &          5x,''!!! Variance  bb('',i3,'') = '',f7.3,'' < 0 !!'')')
     &          stn(m),m,bb(m)
           write(6,'(1x,''WARNING:   Station = '',a4,
     &         5x,''!!! Variance  bb('',i3,'') = '',f7.3,'' < 0 !!'')') 
     &         stn(m),m,bb(m)
           write(6,*)
         endif
         if(iwarn.eq.0) bb(m)=sqrt(bb(m))
         aa(m)=aa(m)/dd(m)
         ee(m)=ee(m)/icc(m)
         if(iwarn.eq.0)then
     ...
     &             stn(m),phz(2),icc(m),ee(m),aa(m),bb(m),dd(m),ptcor(m)
            ...
            write(16,'(1x,a4,3x,a1,i4,2f8.4,''   ?.?.?''1x,2f8.4,
     &              '' <--- !!'')')
     &             stn(m),phz(2),icc(m),ee(m),aa(m),dd(m),ptcor(m)
            iwarn=0
         endif
         if(nsp.eq.1) goto 39
...
c   (nsp=2):
     ...
     &             stcor1
            ...
            write(16,'(1x,a4,3x,a1,i4,2f8.4,''   ?.?.?''1x,2f8.4,
     &              '' <--- !!'')')
     &             stn(m),phz(1),iccs(m),ees(m),aas(m),dds(m),stcor1
            iwarn=0
         endif
